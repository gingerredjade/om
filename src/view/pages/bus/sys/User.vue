  ...
  <div class="page-container">
      ...
      <div class="kt-portlet__body kt-portlet__body--fit">
        ...
        <el-table :data="pageResult.list" stripe size="mini" style="width: 100%;"
          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              <b-s-button v-if="scope.row.status === 1" icon="fa fa-user-o" :tag="$t('STATUS.USER_OK')" variant="success"
                          @click="handleUpdate(scope.row)"
                          perms="sys:user:edit"
                          stylesheet="font-size: 1.0rem;"
              />
              <b-s-button v-else-if="scope.row.status === 2" icon="fa fa-lock" :tag="$t('STATUS.USER_LOCK')" variant="warning"
                          @click="handleUpdate(scope.row)"
                          perms="sys:user:edit"
                          stylesheet="font-size: 1.0rem;"
              />
          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              <b-s-button icon="fa fa-edit" :tag="$t('ACTION.EDIT')"
                          perms="sys:user:edit"
                          @click="handleEdit(scope.$index, scope.row)"
                          stylesheet="font-size: 1.0rem;"
              />
              <b-s-button icon="fa fa-trash" :tag="$t('ACTION.DELETE')" variant="danger"
                          perms="sys:user:delete"
                          @click="handleDelete(scope.row)"
                          stylesheet="font-size: 1.0rem;" class="ml-4"
              />
            </template>
          </el-table-column>
        </el-table>
        ...
        <div style="padding: 10px;">
          <b-s-button :tag="$t('ACTION.BATCHDELETE')" :size="size" variant="danger"
    ...
    <el-dialog :title="operation?$t('ACTION.ADD'):$t('ACTION.EDIT')" width="40%"
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button :size="size" type="primary" @click.native="submitForm" :loading="editLoading" style="color: #fff;">{{$t('ACTION.SUBMIT')}}</el-button>
      </div>
    </el-dialog>
  </div>
</template>
...
<script>
  ...
  import PopupTreeInput from '@/view/components/PopupTreeInputOutBlur';
