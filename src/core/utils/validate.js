...
export function isIP(s) {
  return RegularExpression().getRegularExpress().ipReg.test(s);
}

// 端口验证
export function isPort(s) {
  return RegularExpression().getRegularExpress().portReg.test(s);
}

// 是否含有'*'验证
export function containAsterisk(s) {
  // 规则对象
  const asteriskReg = new RegExp("[*]");
  return asteriskReg.test(s);
}
...
const RegularExpression = function () {
  const regularExpression = {
    ...
    "portReg" : /^([0-9]|[1-9]\d{1,3}|[1-5]\d{4}|6[0-4]\d{4}|65[0-4]\d{2}|655[0-2]\d|6553[0-5])$/, // PORT
    "ip_portReg" : /^(\d{1,2}|1\d\d|2[0-4]\d|25[0-5])\.(\d{1,2}|1\d\d|2[0-4]\d|25[0-5])\.(\d{1,2}|1\d\d|2[0-4]\d|25[0-5])\.(\d{1,2}|1\d\d|2[0-4]\d|25[0-5])\:([0-9]|[1-9]\d{1,3}|[1-5]\d{4}|6[0-5]{2}[0-3][0-5])$/, // IP:Port
